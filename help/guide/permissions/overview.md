...
Esses guias descreverão a configuração do acesso para administradores de sistema, administradores de produtos e usuários finais. Consulte a tabela abaixo para entender a diferença principal entre as funções.
...
| Administrador de produto | Administra os produtos atribuídos a eles e a todas as funções administrativas associadas, como adicionar usuários a organizações, adicionar ou remover usuários de perfis de produtos e adicionar ou remover outros administradores de produtos de um produto. |
| Usuário final | Os usuários na organização que usam os produtos. |
...
Consulte o gráfico abaixo para determinar quem é necessário para configurar os privilégios e onde start com base em seu caso de uso controle de acesso. **Certifique-se de seguir o tutorial até o final do guia do seu local de partida.**

>[!TIP]
>
> Um super usuário refere-se ao mais alto nível de acesso a ser obtido pelo administrador do sistema. Um super usuário pode executar todas as tarefas administrativas e usuário funcionalidade. Um administrador de sistema não tem produtos funcionalidade prontos para uso e precisa fornecer o acesso apropriado, conforme mostrado no gráfico abaixo.
...
| Superusuário para um administrador de sistema do Experience Platform existente **com acesso à interface do usuário do Experience Platform**. | Um administrador de sistema. | [Configurar acesso de colaboração CDP em tempo real](./manage-user-access.md#RTCDP-collab-access) |
...
| Privilégios de administrador de produto e acesso ao Real-Time CDP Collaboration para um novo administrador de produto. | Um administrador de sistema. | [Configurar o acesso do administrador do produto](./manage-user-access.md#admin-access) |
| Acesso de colaboração CDP em tempo real para um administrador **de produto Experience Platform existente com** Experience Platform acesso interface. | Um administrador de sistema ou de produto. | [Configurar acesso de colaboração CDP em tempo real](./manage-user-access.md#RTCDP-collab-access) |
...
### Importação de público-alvo {#audience-importation}

Antes de começar a compartilhar públicos-alvo com colaboradores, você precisa importar públicos-alvo para a Colaboração cdp em tempo real. Atualmente, a única conexão de dados compatível com a importação de públicos é a Experience Platform. Para começar, os usuários que gerenciam a integração de público-alvo precisarão receber uma função com as seguintes permissões de recurso de **Gerenciamento de perfil**:

| Permissão | Descrição |
| ---- | ---- |
| Exibir segmentos | Permite que os usuário vejam o lista de públicos disponíveis em uma área de segurança. |
| Perfis do Exibir | Permite que os usuário vejam os campos disponíveis para mapeamento para campos de colaboração. |

Abaixo, você pode ver um exemplo função com as permissões acima adicionadas. Para obter mais informações sobre como criar ou atribuir funções, consulte o guia gerenciar [de funções](./manage-roles.md) .
