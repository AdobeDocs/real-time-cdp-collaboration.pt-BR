...
>[!IMPORTANT]
>
>Depois que a conexão entre dois colaboradores for estabelecida, as configurações de conexão não poderão ser revisadas.

## Enviar convite {#send-invite}

Para configurar uma conexão, selecione **[!UICONTROL Conectar]** ao navegar pelo inventário de editores no espaço de trabalho **[!UICONTROL Descobrir editores]**.
...
Depois que o convite for enviado, você poderá visualizar (mas não editar) as configurações de conexão. Exibir convite pendente é exibido na guia **[!UICONTROL Minhas conexões]**. O status da conexão aparece como **[!UICONTROL Convite enviado]**.
...
>abstract="Esta seção determina quem está pagando pelas atividades correspondentes no Collaboration."
